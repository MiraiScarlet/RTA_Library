http://www.speedrunslive.com/tools/mm-bingo/

Jimmieさん作成のカスタム版(Seed同値確認済み)

https://bingo.jimmie1717.com/mm?lang=jp

分かりづらいタスクのまとめとかお役立ち情報とか

## ルールについて
- 機種制限なし
- バグ技制限もなし
  - 一部タスクには増殖なし(主にビン・かけら系)などの制限がある場合がある
  
- 以下のアイテムを用意したレースファイルでスタートする(JP Bingo)
  - 時のオカリナ
    -オカリナ**のみ**Cボタンにセット可
  - 時の歌
  - いやしの歌
  - デクナッツの仮面
  - 銀行に101ルピー
  - 時計台前のハートのかけら取得
- レースファイル作成に関し、**暗号系の事前把握は可**

※海外ルールは上記から「いやしの歌」「デクナッツの仮面」を省いたレースファイルを用意する
  
- VCで用意が面倒な場合は以下のセーブファイルを使用するのがおすすめ
  - [Speedrun.com Majora's Mask Resource - Wii VC Race File](https://www.speedrun.com/saves/Race_Files_lxv5j.zip)
  - このセーブファイルは暗号が以下の通りに設定されている
    - ボンバーズ:54321
    - 海のクモ館:赤青赤青赤青
    - 宝くじ屋:全日「111」
    - ※注目がホールドになっているのでスイッチ使用者は注意
  
- タイマースタートはファイルセレクト時
- タイマーストップは全タスク達成時
  - 取得系はアイテムを掲げた時
  - 誓いの号令は「誓いの号令を覚えた」のメッセージが表示されたら
  - 「Rock Sirloin above head」(ロース岩を持ち上げた状態)は必ず最後に達成することになる
  
- 時の歌でリセットされるものに関して
  - アイテム所持系はクリア時に持っておく必要がある
    - ex)「大爆弾」のタスクがある列で、大爆弾入手後に時の歌を吹いた場合、次のサイクルで再入手が必要
  - 仕掛け系(動詞タスク)は一度条件を達成すれば時の歌を吹いても達成扱いになるが、**進行途中で時の歌を吹いた場合最初からやり直しになる**
    - ex)「リザルフォス3体」のタスクで、1体倒して時の歌を吹いた場合は次のサイクルで3体倒さないといけない
      - 3体倒して時の歌を吹いた場合は達成したことになる(要は同一サイクルで1回でも達成していればいい
      - この条件に該当するタスクは、カスタム版において金色で表示される

- 歌x個、お面x個、大妖精x匹系のタスクは指定されている数より多くとっても達成扱いとなる
  - 時オカビンゴは明記されているので、恐らく同じ？
  - 但しはぐれ妖精は時の歌でリセットされるので要注意
  
## お役立ち情報系

- お面各種カンペ
  - https://media.discordapp.net/attachments/310368049708531712/331103872838008833/d00e0e820cca06ef.png?width=624&height=468
  
- nJOY：ゼルダの伝説 ムジュラの仮面(3Dの情報が混ざっているので要注意)
  - https://i-njoy.net/zd6_top.html

## アイテム系

- Razor Sword → フェザーソード
- Gilded Sword → 金剛の剣
- Lullaby Intro → 半分だけの子守唄 **(ゴロンのララバイを取得したら未達成扱い)**
- Magical Mushroom → 魔法のキノコ
- Powder Keg → 大爆弾
- Circus Leader's Mask → 座長のお面
- Couple's Mask → めおとの面
- Bunny Hood → うさぎずきん
- Captain's Hat → 隊長のぼうし
- Mask of Scents → ブーさんのお面
- Biggest Quiver → 最大の矢立て
- Seahorse → タツノオトシゴ
- Chateau Romani → シャトーロマーニ

- Happiness Seal → 団員手帳のリボン(完了印)をつける **※団員手帳の取得必須**
  - 団員手帳:リンクの姿で暗号を伝えて、時計台方面に向かってから引き返すとリーダーから貰える(**不法侵入は不可**)


## タスク系

- Clear All [X] Chest Icons on [Y] Map 
  - Yマップの地図からX個の宝箱消す(**マップが無いと達成扱いにならない**)
  - 宝箱を開けた後にマップを買うことでも達成扱いになる

- Remove All Blocks from SHT Pillar
  - スノーヘッドの神殿の柱に挟まってる氷ブロックを全部押し出す

- 10 Unique Bottle Contents
  - 10個のビンに違う中身を入れて用意する(増殖必須)
  
- Curiosity Shop Owner Happiness Seal
  - マニ屋のおじさん(のリボン(団員手帳))
  
- Diagonal of 4 Item Slots
  - Ｃアイテム画面のアイテムを斜めに揃える
  
- Row of 6 Masks
  - 横一列のお面を揃える
  
- Column of 4 Masks
  - 縦一列のお面を揃える
  
- Explode Hole in Ikana Castle Ceiling
  - イカーナ古城の天井の穴を開ける(要:大爆弾)
  
- Grow 8 Bean Plants
  - 魔法の豆を8つ育てる
  
- Gold Rupee from Ikana Business Scrub
  - わらしべイベントの最後にもらえる200ルピー(月の涙→沼→山→海→谷、要各種仮面)

- Both Shooting Gallery HPs
  - 両方の射的場(街、沼)のハートのかけら

- Open Chest of Magic Beans
  - 魔法の豆が入ってる宝箱を開ける(ロックビル反転後の上段にある豆を育てて行く所にある宝箱)
  
- Ghost Hut HP
  - イカーナ渓谷の幽霊小屋でクリアすると貰えるハートのかけら(要:ハート4つ以上)
 
- Unbar 5 doors in the Well
  - 井戸の下の扉を5箇所開ける(要:ギブドのお面と対応するアイテム各種)
  
- Path to Snowhead HP
  - 山里～スノーヘッドへの道中にあるハートのかけら(正規だとまこと+案山子フックで取るやつ)
  
- Open 2 Graves
  - 墓を2箇所暴く(要:隊長のぼうし)
    - **(JP暫定)** 2箇所暴いた後に時の歌を吹いた場合は、タスク達成状態はリセットされない
    - ※ダイナフォスと同じく、同じサイクルで達成する必要がある
    
- No Hidden Owl Statue
  - 隠しふくろう像禁止(大翼自体は取得可)

- No Song of Soaring
  - 大翼の歌取得禁止

- Blank B Button (no sword)
  - 剣を外す(ライクライクに喰われるorタックリーに盗まれるor鍛冶屋に剣を預ける)
  
- No Shield
  - 盾を外す(ライクライクに喰われるorタックリーに盗まれる)
  
- Enter 4th Day
  - 新しい日の朝を迎える([バグ技](https://www.youtube.com/watch?v=5SKott0Kzoc))
  - (※要確認)一応普通にクリアしてED見ても良かったはず(そんなことする人は誰もいないだろうけど)
  
- Press 3 STT Yellow Floor Switches
  - ロックビルの神殿にある3つの黄色スイッチを押す(要:ゴロンの仮面)
  
- Exactly 333 Rupees in Wallet
  - 333ルピーピッタリ財布に入れる(要:巨人のサイフ)
  
- All Top Row Songs
  - 上段の歌全て((時・いやし・)エポナ・大翼・嵐)
    
- 3 Bottom Row Songs
  - 歌の下段スロットを3つ埋める(半分だけの子守唄可)
  
- Defeat Garo Ninja
  - ガロを倒す(イカーナ渓谷・要:まことのメガネ・ガロのお面)
  
- Rock Sirloin above head
  - ロース岩を掲げる
  
- Change GBT Water Direction
  - グレートベイの神殿の水流の流れを変える

- All 5 Termina Grotto HPs
  - タルミナ平原の穴の中にあるハートのかけら5つ全て 
    - ピーハット、ドドンゴ、隠れアキンドナッツ、ゴシップ、蜂の巣
    
- 5 East Clock Town HP
  - クロックタウン東にあるハートのかけら5つ
    - 町長(めおとの面)、トイレの手(紙)、アンジュのおばさん×2(夜更かし)、ハニーとダーリンのお店(3日連続、ボム袋と弓)、宝箱屋(ゴロン)、射的場パーフェクト2回目(弓)、ポストの中身(ポストハット)
    
- 20 Total Stray Fairies
  - 合計で20匹(以上)のはぐれ妖精を捕まえる(街のはぐれ妖精もカウント)

- Destroy 15 SHT Ice Blocks
  - スノーヘッドの神殿の氷を15個溶かす
    - **(JP暫定)** 出入りで復活する氷(ex:マップがある部屋)を、出入りして繰り返して消すのはカウントしない
